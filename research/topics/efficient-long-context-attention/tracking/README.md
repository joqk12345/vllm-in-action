...
| Kimi K3 on vLLM | KDA/MLA hybrid cache、partial hit、DSpark state、PD/offload correctness、FlashInfer 依赖 | merge commit + image digest + first release + HF revisions |
...
2. 基于当前 release 基线 vLLM `v0.26.0` 建立 Kimi Linear 最小 smoke test；`v0.11.1` 只保留为首个支持入口的历史边界。
...
12. Kimi K3：固定 `vllm/vllm-openai:kimi-k3` 镜像 digest、FlashInfer 版本、Kimi K3/DSpark HF revisions，并跟踪首个包含 PR #50000 的稳定 release。
13. Kimi K3：复现 partial-tail hit、copy-on-write、eviction/reuse、PD tail zeroing 与 DSpark rejection 后 KDA state correctness。
